    ...
    return (
        <div id='opportunity-modal-wrapper'>
            ...
            <ul className='opoortunity-modal-options-bts-list'>
                ...
                {
                    props.hasApplied ? <button className='has-applied-btn'>You applied for this role 6 days ago</button> : props.isPostAdmin ? <></> : <button className='apply-btn'>{props.job.opportunity.application.external ? <span><FontAwesomeIcon icon={faArrowUpRightFromSquare}></FontAwesomeIcon></span> : <></>}Apply</button>
                }
            </ul>
        </div>
    )
